...
- reached by: Home -> favourites rail card OR grid card OR a store-linked banner; ShopsListingScreen -> shop card; Orders -> Past Orders -> t.custReorder; OrderDetailsScreen -> the shop card's chevron circle. HyperlocalScreen -> shop card and NeighbourhoodMapScreen -> shop pin open it as StorePageLayout.dekkane (below).
