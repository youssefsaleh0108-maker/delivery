...
- states: Loading: full-screen CircularProgressIndicator while isLoadingFirstPage || _loadingRails · Error: Icons.cloud_off_rounded + t.couldNotLoadStorefront = "Could not load the storefront" + the raw exception text + Try again · Empty: YdEmptyState t.noShopsMatch = "No shops match", message t.tryClearingAFilter = "Try clearing a filter or two." when filters active, else t.nothingDeliveringHere = "Nothing is delivering here just yet." · No banners / no favourites / no chips: those sections are simply absent, not empty-stated · Shop closed: meta line shows t.statusClosed = "Closed" instead of the ETA and drops the fee · Section subtitle t.shopsDelivering(n) is ICU-plural and uses the SERVER total, except under the Offers chip where it counts what is on screen
  ...
  - [ ] t.isrchSearchHint = "Search shops, cuisines and items" (hint cycles t.custSearchInCategory = "Search in {name}" every 2.8s) — 350ms-debounced server search re-running the grid, and from two characters the items section below. The animated hint freezes the moment you type — check it does.  `YdSearchField (custom)`
  - [ ] Items section (HomeItemSearchSection, home_item_search_section.dart), above the grid heading — t.isrchNearTitle = "Items in shops near you" (t.isrchAnywhereTitle = "Items in shops" when the address has no pin). From 2 typed characters, POST /api/products/search/items with size 3: three shops, each ItemSearchGroupCard with its best two items at the server's price and an AddButton (BasketAdd: shop limit, options -> ProductDetailScreen, else straight into the basket filed under that shop); the shop line opens StorePageScreen through _openStore. t.isrchSeeAll = "See all items" (YdPillButton.secondary) only when there is more -> ItemSearchScreen. Not drawn below 2 characters, while CustomerShell's connectivity says offline, or when the search fails (a refusal, 429 or 503 included); empty -> one line t.isrchEmptyNear = "No open shop near you has “{query}” right now" (t.isrchEmptyAnywhere without a pin; closed shops and sold-out items are left out, so never "no shop sells it"), plus t.isrchEmptyTruncated(limit) under it when the server stopped at its ceiling. Driven through CustomerShell by test/item_search_test.dart, alone at 320dp and in Arabic.  `HomeItemSearchSection in a SliverToBoxAdapter`
  ...
  - [ ] t.dekkaneBrowseTitle = "Your neighbourhood" / t.dekkaneEntrySub = "The dekkanes and small shops around your address" — Pushes HyperlocalScreen (Figma 112:1941) over the shell with the shell's cart, address book, zoneApi and onOpenBasket. Drawn only once the feed has loaded, never over the spinner or the error state. Shares one row with the neighbourhood chat tile (t.chatRoomEntryTitle, pushes NeighbourhoodChatScreen): two equal half-width tiles under the banner rail, or in the rail's place when there are no banners. The row first measures the longest word of both titles at the reader's text size. When one would not fit a half tile, which would break it mid-word, the two are stacked full-width cards instead, still under the rail. In Rubik that happens at text x1.3 on 320dp and 360dp phones; at 390dp they stay side by side up to x1.3. Titles are never cut short in either shape. With no chat client the browse is the full-width card on its own. The chevron is Icons.chevron_right in both directions (the icon mirrors itself in RTL). Driven through CustomerShell by test/hyperlocal_screen_test.dart. Side by side vs stacked, and word breaks at 320/360/390dp and text x1/x1.3/x2, are covered in the app's own font by test/store_home_screen_rubik_test.dart. Overflow in the full-em test font, and Arabic, by test/store_home_screen_test.dart.  `YdCard half-width tiles in an IntrinsicHeight Row (LayoutBuilder outside it), or two stacked full-width YdCards, under the banner rail`
...
### ItemSearchScreen (every shop that sells it)
*The full item search behind Home's "See all items": a search field with the words, then every shop that sells a match, a page at a time.*

- file: `D:/workspace/delivery/clients/apps/mobile_app/lib/src/item_search_screen.dart` (cards in item_search_widgets.dart)
- reached by: Home -> type 2+ characters -> items section -> t.isrchSeeAll = "See all items".
- covered by: test/item_search_test.dart — pin in the body, distance only with a pin, Add into the basket under its shop, options open ProductDetailScreen, the fourth shop's shop-limit dialog, "N more in this shop", empty and truncated, failure, 320dp, Arabic RTL.
- states: Loading: spinner · Failed first page (a 500, or 429 SEARCH_RATE_LIMITED / 503 SEARCH_TIMED_OUT): t.isrchCouldNotSearch + t.tryAgain · Words the server will not search (ItemSearchRefusal, no retry): SEARCH_TOO_SHORT ("a.", "1 l") -> t.isrchTypeMore, SEARCH_TOO_MANY_WORDS / SEARCH_TOO_LONG -> t.isrchUseFewerWords = "Use up to 5 words to search items." · Field under 2 characters: t.isrchTypeMore · Empty: t.isrchEmptyNear = "No open shop near you has “{query}” right now" / t.isrchEmptyAnywhere, plus t.isrchEmptyTruncated(limit) when the server stopped at its ceiling · No pin: the line t.isrchNotByDistance under the field, no distances.
...
- reached by: Home -> favourites rail card OR grid card OR a store-linked banner; ShopsListingScreen -> shop card; ItemSearchScreen -> shop line, or "N more in this shop" (opens with initialSearch: shelf searched for the same words); Orders -> Past Orders -> t.custReorder; OrderDetailsScreen -> the shop card's chevron circle. HyperlocalScreen -> shop card and NeighbourhoodMapScreen -> shop pin open it as StorePageLayout.dekkane (below).
- dekkane layout (Figma 112:2041, driven by test/dekkane_shop_test.dart): the same state, paging, option sheet, one-shop rule and basket bar, drawn as the frame — YdScreenHeader with the shop's name, its district as the subtitle (omitted when it has none) and YouDropPill; an inset hero with the open pill t.dekkaneOpenClosesAt(time) (else the availability label), the power pill (DekkanePowerPill: generator or dark with how long ago it was declared, nothing for mains, undeclared or a declaration no longer current) and rating + t.custRatingsCount + tagline; aisle chips starting at t.all; t.dekkaneShopInventory over a two-column ShelfGridTile grid whose t.dekkaneLbpAmount line exists only once a rate is loaded, with no Add on a closed shop. No tabs, in-shop search or heart. The "Chat with the shop" pill is NOT drawn: StorePageScreen.shopChatAction is the hook (null today), and whatever it returns becomes the Scaffold's floatingActionButton, above the basket bar.
...
### HyperlocalScreen (Your neighbourhood, Figma 112:1941)  — reachable from Home; driven by test/hyperlocal_screen_test.dart
*The dekkanes around the customer's delivery address, nearest first, from GET /api/stores/nearby (2 km). Rebuilt to the redesign: the district chips and arabizi search the old screen had are gone, replaced by attribute chips the server filters on.*

- file: `D:/workspace/delivery/clients/apps/mobile_app/lib/src/hyperlocal_screen.dart` (+ `neighbourhood_map_screen.dart`)
- reached by: Home (StoreHomeScreen) → the "Your neighbourhood" card under the banner rail → pushed over the shell with the shell's cart, address book and onOpenBasket.
- states: loading; first-page error with Try again; empty (filtered → "No shops match"; unfiltered → "No shops near this address yet"; when the server says the search reached its 500-candidate ceiling the message is t.dekkaneSearchedNearest(500) instead of "try clearing a filter", and the same line ends a list); load-more footer and its retry; no pinned address → the storefront browse, titled t.dekkaneBrowseTitleAll = "Shops on YouDrop" / t.dekkaneBrowseSubAll / section t.dekkaneAllShops, with a "pin your address" prompt and no chips, map or distances; a shop that is not open wears its state (Closed / Closing soon / Busy) on its cover; closed shops and shops currently declared dark are dimmed.

  - [ ] t.dekkaneBrowseTitle / subtitle t.dekkaneBrowseSubRegion(region) (falls back to t.custHyperlocalSub) + YouDropPill; without a pinned point t.dekkaneBrowseTitleAll / t.dekkaneBrowseSubAll — `YdScreenHeader`
  - [ ] Location bar (area, region) — opens the address sheet; a new address reloads the list. Drawn ~34px tall; the 12px band under it opens the sheet too (46px target).  `InkWell` + `GestureDetector`
  - [ ] Chips All / Open now / On generator now / New on YouDrop — nearby's openNow / powerStatus=GENERATOR (matches only a declaration still current: delivery.product.power-declaration-fresh-for, 4 h) / newSinceDays=30 (counted from stores.published_at, the first listing). Every filter but openNow narrows the server's 500 candidates in SQL. The frame's "Has Generator" is asked as "on generator now" (power_status is the current state, not ownership); its "Delivers" chip is not drawn (no agreed meaning).  `YdChip`
