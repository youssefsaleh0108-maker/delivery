...
- states: loading; first-page error with Try again; empty (filtered → "No shops match"; unfiltered → "No shops near this address yet"); load-more footer and its retry; no pinned address → storefront browse with a "pin your address" prompt and no chips, map or distances; dark shops dimmed.

  - [ ] t.dekkaneBrowseTitle / subtitle t.dekkaneBrowseSubRegion(region) (falls back to t.custHyperlocalSub) + YouDropPill — `YdScreenHeader`
  - [ ] Location bar (area, region) — opens the address sheet; a new address reloads the list.  `InkWell`
  - [ ] Chips All / Open now / On generator now / New on YouDrop — nearby's openNow / powerStatus=GENERATOR / newSinceDays=30. The frame's "Has Generator" is asked as "on generator now" (power_status is the current state, not ownership); its "Delivers" chip is not drawn (no agreed meaning).  `YdChip`
  - [ ] Map preview (100px, still) → "Expand interactive map" → NeighbourhoodMapScreen, pins open the shop. Hidden without a point or without shops; says "map unavailable" and drops the pill when tiles fail.  `OsmBasemap`
  - [ ] Shop cards: cover, Trusted local (verified_local, Backoffice-granted via PUT /api/stores/{id}/verified-local), fact line, rating or "New", distance, generator / dark pill → StorePageScreen in StorePageLayout.dekkane (Figma 112:2041).  `DekkaneShopCard`
