...
class PortalArea {
  ...
  /// Cash custody — the reconciliation overview (Figma 112:9) and the rider settlement it opens —
  /// is next: append its page here and the heading grows a row for each page, with nothing else in
  /// the shell to change. The heading opens whichever page is first.
  ///
  ...
  ///
  /// EXTENSION POINT for per-rider HR pages: a rider's attendance month (Figma 112:945), their pay
  /// history. Empty until the first of them exists, because a button with nothing behind it is a
  /// dead control. A function of the clients rather than a list, so an entry can hand its page
  /// whatever it reads without threading clients through the directory and the profile.
  static List<RiderPage> carrierRiderPages(PortalApis apis) => const <RiderPage>[];
