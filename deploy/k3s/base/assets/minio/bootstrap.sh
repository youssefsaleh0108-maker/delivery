...
# The dev-build download drop (added for the k3s environments): holds nothing but the installable
# APK, which every phone that installs it gets anyway — so anonymous download is the point.
echo "==> Creating apk download bucket"
mc mb --ignore-existing local/apk
mc anonymous set download local/apk
