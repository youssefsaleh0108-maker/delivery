...
demo_client() { [ "$1" = backoffice ] && echo delivery-portal || echo mobile-app; }
...
kc_login() {   # a fresh admin token, into a header file; the admin password goes by file, never argv
  ...
  curl -s -X POST "$IAM/realms/master/protocol/openid-connect/token" -d client_id=admin-cli \
  ...
  rm -f "$WORK/kc-pass"
  [ -s "$WORK/kc-token" ] || die "no Keycloak admin token from $IAM"
